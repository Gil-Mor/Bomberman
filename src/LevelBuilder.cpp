...
Board::GameObjects
LevelBuilder::getLevel(size_t level, const size_t numOfPlayers, size_t numOfEnemies, const Posf& windowSize) const
{
    try {
            ...
            for (size_t j = 0; j < file.at(i).size(); ++j) {
                ...
                switch (file.at(posb.i).at(posb.j)) {
                    ...
                    case PLACE_HOLDER_SYMBOL:
                        ...
                        objects.board.at(posb.i).at(posb.j).emplace_front(
                            new FreeTile(objects.posfMap.at(posb.i).at(posb.j), posb));
                    ...
                    case STONE_WALL_SYMBOL:
                        objects.board.at(posb.i).at(posb.j).emplace_front(
                            new StoneWall(objects.posfMap.at(posb.i).at(posb.j), posb));
                        objects.FreeTiles.at(posb.i).at(posb.j) = false;
                        break;
                    ...
                    case TELEPORT_SYMBOL: {
                        ...
                        objects.board.at(posb.i).at(posb.j).emplace_front(
                            new Teleport(objects.posfMap.at(posb.i).at(posb.j), posb, id));
...
void LevelBuilder::fillWithBomberMan(size_t numOfPlayers, size_t numOfEnemies, Board::GameObjects& objects) const
{
    try {
        ...
        if (numOfEnemies >= 1) {
            ...
            /*if (level == 1) {
            enemy.reset(new StupidEnemy(objects.posfMap[bottomRight.y][bottomRight.x],
            bottomRight));
            }*/
            ...
            objects.enemies.push_back(static_cast<AIEnemy*>(objects.board.at(pos.i).at(pos.j).begin()->get()));
        }
...
void LevelBuilder::coupleTeleports(Board::GameObjects& objects, vector<Posb>& _teleports) const
{
    try {
        int id1, id2;
        Teleport *tmp1, *tmp2;
