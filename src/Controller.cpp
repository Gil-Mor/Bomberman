...
Controller::Controller() :
    _proxy(*this), _mainMenu(_proxy), _gameMenu(_proxy), _pauseMenu(_proxy), _levelsMenu(_proxy),
    _instructionPages(_proxy), _level(_proxy)
{
    try {
...
void Controller::loadNextLevel()
{
    try {
        _level.setLevel(LevelBuilder::getInstance().getLevel(
            _levelProporties.currLevel, _levelProporties.numOfPlayers, _levelProporties.numOfEnemies, getWindowSize()));
