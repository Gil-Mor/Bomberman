...
Path AIMap::pathToDestination(const Posb& source, const Posb& dest)
{
    ...
    boost::breadth_first_search(
        _graph, _path.source->getDescriptor(), boost::visitor(BFSVisitor(AIMapBFSProxy(*this, DESTINATION_C))));
...
Path AIMap::pathToSaftey(const Posb& source)
{
    ...
    boost::breadth_first_search(
        _graph, _path.source->getDescriptor(), boost::visitor(BFSVisitor(AIMapBFSProxy(*this, SAFE_C))));
...
bool AIMap::validPos(size_t i, size_t j) const
{
    ...
    return i < _tiles.size() && j < _tiles[0].size();
}
